...
if(is_array($datos)){
    ...
    if( $id > 0){
        ...
        $productos = isset($_SESSION['carrito']['medicamentos']) ? $_SESSION['carrito']['medicamentos'] :  null;

        if ($productos != null) {
            foreach($productos as $clave => $cantidad){
        
                $sql = $con->prepare("SELECT id, nombre, precio, descuento FROM medicamentos WHERE id=? AND activo=1");
                $sql->execute([$clave]);
