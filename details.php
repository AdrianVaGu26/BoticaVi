...
if ($id == '' || $token == ''){
    ...
    if($token == $token_tmp){
        ...
        $sql = $con->prepare("SELECT count(id) FROM medicamentos WHERE id=? AND activo=1");
        $sql->execute([$id]);
        if($sql->fetchColumn() > 0) {

            $sql = $con->prepare("SELECT nombre, descripcion, precio, descuento  FROM medicamentos WHERE id=? AND activo=1 LIMIT 1");                                            
            $sql->execute([$id]);
            ...
            $rutaImg = $dir_images . 'digestal.jpg';

            if(!file_exists($rutaImg)) {
                $rutaImg= 'images/productos/2/digestal.jpg';
            }
            ...
            while(($archivo = $dir->read()) != false) {
                if($archivo != 'digestal.jpg' && (strpos($archivo, 'jpg') || strpos($archivo, 'jpeg'))) {
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarHeader">
                ...
                <a href="carrito.php" class="btn btn-primary btn-sm me-2">
